...
第 10 轮已经落地轻量复审闭环：人工 `NEED_MORE_INFO` 后，审批人员录入补件摘要，状态进入 `MATERIAL_UPDATED`；再点击重新提交，进入 `RESUBMITTED`；之后必须重新触发 AI Review，回到 `AI_REVIEWED` 后才能人工 approve/reject。补件只保存 mock 摘要，不上传真实敏感材料。

### 0.6 NEED_MORE_INFO 为什么不再是简单终态？

真实信贷业务里，`NEED_MORE_INFO` 更像“需要客户补充材料”的中间状态，而不是业务结束。把它接到 `MATERIAL_UPDATED` 和 `RESUBMITTED`，可以展示状态机设计能力，也能解释补件、重提、复评和人工终审的完整闭环。

### 0.7 重新 AI review 会覆盖旧报告吗？

不会。Java 后端每次 AI Review 都新增一条 `ai_decision_report`，通过 report id 和 workflow id 区分轮次。这样可以追溯补件前后的不同 AI 建议，避免旧报告被覆盖后无法审计。

### 0.8 补件流程中 AI 和人工边界怎么保证？

补件和重提由 Java 业务接口写状态并记录 audit log；AI Review 只把申请从 `SUBMITTED` 或 `RESUBMITTED` 推进到 `AI_REVIEWED`，生成辅助建议和报告。最终 `APPROVED` / `REJECTED` 仍只能由人工审批接口写入，LLM 没有最终审批工具。

### 0.9 这套状态机和普通 CRUD 有什么区别？

普通 CRUD 只是增删改查字段；这里的状态机表达了信贷审批业务约束：哪些状态能 AI review，哪些状态能补件，哪些状态能重提，哪些状态必须人工终审。非法跳转会被 Service 层拒绝，并通过 approval record、material update record 和 audit log 留痕。
