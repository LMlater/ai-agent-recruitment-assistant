...
我会打开 `http://localhost:8080/demo.html` 做完整流程展示。这个页面不是生产前端，只是本地面试 Demo，用原生 HTML、CSS、JavaScript 调 Java 后端 API。流程是先检查 backend 和 agent-service 状态，再初始化并登录 demo admin，创建脱敏客户和贷款申请，提交后触发 AI Review，然后查看 AI Report、RAG 制度引用和 5 个 Agent logs，最后点击人工 Approve、Reject 或 Need More Info 确认最终审批状态。
