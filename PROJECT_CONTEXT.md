...
- 当前项目处于冻结交付状态：第 8-12 轮业务和工程成果已经保留，本轮只做发布前验收、CI 可触发性、Docker Compose 静态校验和最终验收清单。
- 后续不再主动扩展业务功能，不改审批状态机、Agent 主流程或数据库业务表。
- 后续只建议做 bugfix、CI/Docker 验证、README/简历表达调整，以及真实演示中暴露的问题修复。
- 继续保持默认 Mock LLM、不提交 `.env` 或真实 API Key、不接真实银行数据、不让 AI/LLM/Agent 自动写最终审批状态。
